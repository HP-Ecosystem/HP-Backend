...
from core.exceptions import BadRequestError, BaseAPIException, ConflictError
...
class UserManager(BaseUserManager):
    ...
    def _create_user(
        ...
        """
        if not email:
            raise BadRequestError(detail="Email address is required")
        ...
        try:
            ...
            if "email" in str(e):
                ...
                raise ConflictError(
                    detail="User with provided credential already exists"
                ) from e
    ...
    def create_user(
        ...
        **extra_fields,
        ...
        if extra_fields.get("is_staff") is True:
            raise BadRequestError(detail="Regular users cannot be staff")

        if extra_fields.get("is_superuser") is True:
            raise BadRequestError(detail="Regular users cannot be superuser")
    ...
    def create_superuser(self, email: str, password: str, **extra_fields) -> "User":
        ...
        if extra_fields.get("is_staff") is not True:
            raise BadRequestError(detail="Superusers must have 'is_staff=True")

        if extra_fields.get("is_superuser") is not True:
            raise BadRequestError(detail="Superusers must have 'is_superuser=True'")
    ...
    def _create_profile(self, user: "User") -> None:
        ...
        if profile_class:
            try:
                ...
                logger.exception(f"Error creating profile for user {user.email}: {e}")
                raise BaseAPIException(
                    detail=f"Failed to create profile for user {user.email}"
                ) from e
